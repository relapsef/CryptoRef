...
export const topUsers = Array.from(
  referrals.reduce((map, ref) => {
    const current = map.get(ref.user) ?? { user: ref.user, referrals: 0, likes: 0, clicks: 0 };
    current.referrals += 1;
    current.likes += ref.likes;
    current.clicks += ref.clicks;
    map.set(ref.user, current);
    return map;
  }, new Map<string, { user: string; referrals: number; likes: number; clicks: number }>()).values()
